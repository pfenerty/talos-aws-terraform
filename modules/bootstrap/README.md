...
| [kubernetes_config_map_v1.cluster_versions](https://registry.terraform.io/providers/hashicorp/kubernetes/latest/docs/resources/config_map_v1) | resource |
...
| karpenter | Karpenter provisioning policy, published into the karpenter-config secret<br/>for the NodePool in the Flux bootstrap repository to read.<br/><br/>`capacity_types` is the list of EC2 purchase options the NodePool may<br/>provision, in no particular order - Karpenter's own price-capacity-optimized<br/>strategy decides between them, and listing spot first does not prefer it.<br/>The default allows both, which is what makes elastic capacity cost a<br/>fraction of the baseline; drop to ["on-demand"] for workloads that cannot<br/>absorb a two-minute interruption notice.<br/><br/>Spot is safe to allow here by construction: this module creates the<br/>interruption queue and the EventBridge rules that feed it, so Karpenter<br/>sees the reclamation notice and drains the node rather than losing it.<br/><br/>This only sets what Terraform publishes. The NodePool has to read it -<br/>see "Node autoscaling" in the README for the valuesFrom entry. | <pre>object({<br/>    capacity_types = optional(list(string), ["spot", "on-demand"])<br/>  })</pre> | `{}` | no |
