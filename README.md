...
on the machine as far as Talos is concerned, and the namespace is the only
granularity Talos offers, so it has to hold nothing but tuppr and it has to
match the chart's release namespace exactly.

```hcl
kubernetes_talos_api_access = { enabled = true }   # namespace: tuppr-system
```
...
```yaml
# clusters/<project_name>/tuppr-system/upgrades.yaml
